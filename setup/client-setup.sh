...
function install() {
  ...
  if [ -f "${CWD}/src/client.py" ]; then
    ...
    curl -L "https://raw.githubusercontent.com/panelssh/simple-server-status/main/src/client.py" | sed -e "0,/^SERVER_HOST = .*$/s//SERVER_HOST = \"${SERVER}\"/" \
      -e "0,/^SERVER_PORT = .*$/s//SERVER_PORT = ${PORT}/" \
      -e "0,/^HOSTNAME = .*$/s//HOSTNAME = \"${HOSTNAME}\"/" "${CWD}/src/client.py" >"$PY_SRC"
  fi
  ...
  if [ "$KERNEL" == "systemd" ]; then
    ...
    if [ -f "${CWD}/setup/client-server.systemd" ]; then
      ...
      curl -L "https://raw.githubusercontent.com/panelssh/simple-server-status/main/setup/client-server.systemd" | sed -e "s|^User=$|User=${RUN_AS}|" \
    ...
    if [ -f "${CWD}/setup/client-server.sysvinit" ]; then
      ...
      curl -L "https://raw.githubusercontent.com/panelssh/simple-server-status/main/setup/client-server.sysvinit" | sed -e "0,/^DAEMON=.*$/s//DAEMON=\"${PY_DIST}\"/" \
